...
export class TaxTotalsComponent extends Component {
    ...
    formatData(props) {
        ...
        debugger;
        let hsRetailTax = 0;
        if (props.record.data.order_line){
            for (const record of props.record.data.order_line.records) {
                if (record.data) {
                    const retail_tax = record.data.retail_tax;
                    const product_uom_qty = record.data.product_uom_qty;

                    if (retail_tax && product_uom_qty) {
                        const total = retail_tax * product_uom_qty;
                        hsRetailTax += total;
                    }
                }
            }
        }
        if (props.record.data.invoice_line_ids){
            for (const record of props.record.data.invoice_line_ids.records) {
                if (record.data) {
                    const retail_tax = record.data.retail_tax;
                    const quantity = record.data.quantity;

                    if (retail_tax && quantity) {
                        const total = retail_tax * quantity;
                        hsRetailTax += total;
                    }
                }
            }
        ...
        if (hsRetailTax){
            finalGSt = hsRetailTax
            amountGstTax = formatMonetary(finalGSt, currencyFmtOpts);
            this.amountGstTax = amountGstTax;
        }
        else{
                finalGSt = this.GST;
        }
        ...
        const customTotal = amount_untaxed + finalGSt
        let rounding_amount = totals.display_rounding && totals.rounding_amount || 0;
        let amount_total = amount_untaxed + finalGSt;
       if (isNaN(amount_total)) {
            amount_total = 0;
        }
        totals.amount_total = customTotal;
        ...
        for (let group_name of Object.keys(totals.groups_by_subtotal)) {
            ...
            for (let key in group) {
                group[key].formatted_tax_group_amount = this.amountGstTax;
